...
// Network form dirty tracking (prevents auto-refresh from overwriting user edits)
...
function setSectionInputsDisabled(sectionId, disabled) {
    ...
    section.classList.toggle('disabled-section', disabled);
}

// Global variables
const BoardSerial = "KC868-A8H73CR36SMIA8";
let systemData = {};
let analogHistory = {
    labels: [],
    datasets: [{}, {}, {}, {}]
...
function updateDashboard(data) {
    ...
    document.getElementById('dash-wifi-ip').textContent = (data.wifi_connected && data.wifi_ip && data.wifi_ip !== '0.0.0.0') ? 
        data.wifi_ip : 'Not connected';
    
    document.getElementById('dash-eth-ip').textContent = (data.eth_connected && data.eth_ip && data.eth_ip !== '0.0.0.0') ? 
        data.eth_ip : 'Not connected';
    
    // Ensure MAC address displays properly
    const macAddress = data.board_mac || data.mac || '';
...
function generateAndDisplaySerialNumber(data) {
    ...
    const serialNumber = `${BoardSerial}-${deviceId}`;
        ...
        link.addEventListener('click', function(e) {
            ...
            if (targetSection) {
                ...
                console.log(`Activated section: ${targetId}`);
                                ...
                                // If Alexa integration section, load data
                if (targetId === 'alexa-integration') {
                    loadAlexaConfig();
                }
...
function initWebSocket() {
    ...
    const storedWsPort = parseInt(localStorage.getItem('ws_port') || '81', 10);
    const wsPort = (storedWsPort >= 1 && storedWsPort <= 65535) ? storedWsPort : 81;
...
function initNetworkUI() {
    ...
    // Mark form dirty on any user edit so periodic refresh doesn't overwrite inputs
    const netInputs = document.querySelectorAll('#network-settings-form input, #wifi-connect-form input');
    netInputs.forEach(el => {
        if (el.readOnly) return;
        el.addEventListener('input', markNetworkDirty);
        el.addEventListener('change', markNetworkDirty);
    });
    ...
    if (refreshNetworkBtn) {
        refreshNetworkBtn.addEventListener('click', function(){ clearNetworkDirty(); fetchNetworkSettings(); });
    }
    
    // Ethernet DHCP toggle
    const ethDhcpToggle = document.getElementById('eth-dhcp-mode');
    if (ethDhcpToggle) {
        ethDhcpToggle.addEventListener('change', function() {
            markNetworkDirty();
            // Keep section visible; disable inputs when DHCP is enabled
            setSectionInputsDisabled('eth-static-settings', ethDhcpToggle.checked);
});
    }

    // WiFi DHCP toggle
    const wifiDhcpToggle = document.getElementById('wifi-dhcp-mode');
    if (wifiDhcpToggle) {
        wifiDhcpToggle.addEventListener('change', function() {
            markNetworkDirty();
            setSectionInputsDisabled('wifi-static-settings', wifiDhcpToggle.checked);
});
    }
...
function fetchNetworkSettings() {
    // Get full network configuration + live status
    fetch('/api/network/settings')
        .then(response => response.json())
        .then(data => {
            // Save WS port locally so the next reload connects correctly
            if (data.ws_port) {
                localStorage.setItem('ws_port', String(data.ws_port));
            }
            ...
            // Ethernet form
            const ethDhcp = document.getElementById('eth-dhcp-mode');
            if (ethDhcp && !networkFormDirty) ethDhcp.checked = (data.eth_dhcp_mode !== false);
            const ethStatic = document.getElementById('eth-static-settings');
            if (ethDhcp) setSectionInputsDisabled('eth-static-settings', ethDhcp.checked);

            const setVal = (id, val, force=false) => {
                const el = document.getElementById(id);
                if (!el) return;
                // Don't overwrite user edits during periodic refresh
                if (!force && networkFormDirty && !el.readOnly) return;
                // Also avoid overwriting while focused
                if (!force && document.activeElement === el && !el.readOnly) return;
                el.value = (val ?? '');
            };
            setVal('eth-ip', data.eth_static_ip);
            setVal('eth-gateway', data.eth_static_gateway);
            setVal('eth-subnet', data.eth_static_subnet);
            setVal('eth-dns1', data.eth_static_dns1);
            setVal('eth-dns2', data.eth_static_dns2);
            setVal('eth-mac', data.eth_mac, true);
            setVal('eth-speed', data.eth_speed || 'Unknown', true);
            setVal('eth-duplex', data.eth_duplex || 'Unknown', true);

            // Ports
            setVal('http-port', data.http_port);
            setVal('ws-port', data.ws_port);

            // WiFi form
            setVal('wifi-ssid', data.wifi_ssid);
            setVal('wifi-password', data.wifi_password);
            const wifiDhcp = document.getElementById('wifi-dhcp-mode');
            if (wifiDhcp && !networkFormDirty) wifiDhcp.checked = (data.wifi_dhcp_mode !== false);
            const wifiStatic = document.getElementById('wifi-static-settings');
            if (wifiDhcp) setSectionInputsDisabled('wifi-static-settings', wifiDhcp.checked);

            setVal('wifi-ip', data.wifi_static_ip);
            setVal('wifi-gateway', data.wifi_static_gateway);
            setVal('wifi-subnet', data.wifi_static_subnet);
            setVal('wifi-dns1', data.wifi_static_dns1);
            setVal('wifi-dns2', data.wifi_static_dns2);
            setVal('wifi-sta-mac', data.wifi_sta_mac, true);
            setVal('wifi-ap-mac', data.wifi_ap_mac, true);
        })
        .catch(error => {
...
function updateNetworkStatusDisplay(data) {
    ...
    // DHCP status badge (shows both Ethernet + WiFi)
    const dhcpStatus = document.getElementById('dhcp-status');
    if (dhcpStatus) {
        const eth = (data.eth_dhcp_mode !== false) ? 'ETH: DHCP' : 'ETH: Static';
        const wifi = (data.wifi_dhcp_mode !== false) ? 'WiFi: DHCP' : 'WiFi: Static';
        dhcpStatus.innerHTML = `<span class="badge badge-info">${eth}</span> <span class="badge badge-info">${wifi}</span>`;
    }
        ...
        if (data.eth_connected) {
            ethStatus.innerHTML = `
                <div class="status-card success">
                    ...
                    <p><strong>Subnet:</strong> ${data.eth_subnet || 'Unknown'}</p>
                    <p><strong>Gateway:</strong> ${data.eth_gateway || 'Unknown'}</p>
                    <p><strong>MAC Address:</strong> ${data.eth_mac || 'Unknown'}</p>
                    <p><strong>Speed:</strong> ${data.eth_speed || 'Unknown'}</p>
                    <p><strong>Duplex:</strong> ${data.eth_duplex || 'Unknown'}</p>
                </div>
            `;
        } else if (data.eth_link_up) {
            ethStatus.innerHTML = `
                <div class="status-card warning">
                    <h4>Ethernet Link Up (No IP)</h4>
                    <p><strong>MAC Address:</strong> ${data.eth_mac || 'Unknown'}</p>
                    <p><strong>Speed:</strong> ${data.eth_speed || 'Unknown'}</p>
                    <p><strong>Duplex:</strong> ${data.eth_duplex || 'Unknown'}</p>
                    <p>Link is detected but no IP address was assigned (check DHCP or use static IP).</p>
                </div>
            `;
        } else {
            ethStatus.innerHTML = `
        ...
        if (data.wifi_client_mode) {
            wifiStatus.innerHTML = `
                <div class="status-card success">
                    ...
                    <p><strong>Subnet:</strong> ${data.wifi_subnet || 'Unknown'}</p>
                    <p><strong>Gateway:</strong> ${data.wifi_gateway || 'Unknown'}</p>
                    <p><strong>STA MAC:</strong> ${data.wifi_sta_mac || 'Unknown'}</p>
            ...
            wifiStatus.innerHTML = `
                <div class="status-card warning">
                    ...
                    <p><strong>AP MAC:</strong> ${data.wifi_ap_mac || 'Unknown'}</p>
                    <p>WiFi client mode is disconnected.</p>
                </div>
            `;
        } else {
            wifiStatus.innerHTML = `
...
function connectToWiFi() {
    ...
    const wifiDhcp = document.getElementById('wifi-dhcp-mode')?.checked !== false;
    const wifiData = {
        wifi_ssid: ssid,
        wifi_password: password,
        wifi_dhcp_mode: wifiDhcp
    };

    if (!wifiDhcp) {
        wifiData.wifi_static_ip = document.getElementById('wifi-ip')?.value || '';
        wifiData.wifi_static_gateway = document.getElementById('wifi-gateway')?.value || '';
        wifiData.wifi_static_subnet = document.getElementById('wifi-subnet')?.value || '';
        wifiData.wifi_static_dns1 = document.getElementById('wifi-dns1')?.value || '';
        wifiData.wifi_static_dns2 = document.getElementById('wifi-dns2')?.value || '';
    }
...
function saveNetworkSettings() {
    const ethDhcp = document.getElementById('eth-dhcp-mode')?.checked !== false;

    const networkData = {
        // Ethernet
        eth_dhcp_mode: ethDhcp,
        // WiFi credentials + mode
        wifi_ssid: document.getElementById('wifi-ssid')?.value || '',
        wifi_password: document.getElementById('wifi-password')?.value || '',
        wifi_dhcp_mode: document.getElementById('wifi-dhcp-mode')?.checked !== false,
        // Web UI ports (Option A)
        http_port: parseInt(document.getElementById('http-port')?.value || '80', 10),
        ws_port: parseInt(document.getElementById('ws-port')?.value || '81', 10)
    };

    // Fix A: send the keys the firmware expects
    if (!ethDhcp) {
        networkData.eth_static_ip = document.getElementById('eth-ip')?.value || '';
        networkData.eth_static_gateway = document.getElementById('eth-gateway')?.value || '';
        networkData.eth_static_subnet = document.getElementById('eth-subnet')?.value || '';
        networkData.eth_static_dns1 = document.getElementById('eth-dns1')?.value || '';
        networkData.eth_static_dns2 = document.getElementById('eth-dns2')?.value || '';
    }

    if (networkData.wifi_dhcp_mode === false) {
        networkData.wifi_static_ip = document.getElementById('wifi-ip')?.value || '';
        networkData.wifi_static_gateway = document.getElementById('wifi-gateway')?.value || '';
        networkData.wifi_static_subnet = document.getElementById('wifi-subnet')?.value || '';
        networkData.wifi_static_dns1 = document.getElementById('wifi-dns1')?.value || '';
        networkData.wifi_static_dns2 = document.getElementById('wifi-dns2')?.value || '';
    }
        ...
        if (data.status === 'success') {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    const portPart = (networkData.http_port && networkData.http_port != 80) ? (':' + networkData.http_port) : '';
                    window.location.href = `${proto}//${host}${portPart}/`;
                }, 15000);
            }, 2000);
        } else {
...
// Initialize Alexa integration UI
function initAlexaUI() {
    console.log("Initializing Alexa UI");
    
    // Load Alexa configuration
    loadAlexaConfig();
    
    // Setup event handlers
    const alexaEnabledCheckbox = document.getElementById('alexa-enabled');
    if (alexaEnabledCheckbox) {
        alexaEnabledCheckbox.addEventListener('change', function() {
            updateAlexaStatus(this.checked);
        });
    }
    
    const saveAlexaConfigBtn = document.getElementById('save-alexa-config');
    if (saveAlexaConfigBtn) {
        saveAlexaConfigBtn.addEventListener('click', saveAlexaConfig);
    }
    
    const discoverDevicesBtn = document.getElementById('discover-alexa-devices');
    if (discoverDevicesBtn) {
        discoverDevicesBtn.addEventListener('click', triggerAlexaDiscovery);
    }
    
    // Close device edit modal when close button is clicked
    const closeButtons = document.querySelectorAll('#edit-alexa-device-modal .close-modal, #edit-alexa-device-modal .close-btn');
    closeButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            document.getElementById('edit-alexa-device-modal').style.display = 'none';
        });
    });
    
    // Setup form submission for device edit
    const editDeviceForm = document.getElementById('edit-alexa-device-form');
    if (editDeviceForm) {
    ...
    // Update device name example in real-time
    const deviceNameInput = document.getElementById('edit-device-name');
    if (deviceNameInput) {
        deviceNameInput.addEventListener('input', function() {
            document.querySelector('.device-name-example').textContent = this.value || 'Device Name';
        });
    }
}

// Load Alexa configuration from the server
function loadAlexaConfig() {
    fetch('/api/alexa/config')
        .then(response => response.json())
        .then(data => {
            // Update UI with configuration
            const enabledCheckbox = document.getElementById('alexa-enabled');
            if (enabledCheckbox) {
                enabledCheckbox.checked = data.enabled;
                updateAlexaStatus(data.enabled);
            }
            
            // Render device list
            renderAlexaDeviceList(data.devices);
        })
        .catch(error => {
            console.error('Error loading Alexa configuration:', error);
            showToast('Failed to load Alexa configuration', 'error');
        });
}

// Render the Alexa device list
function renderAlexaDeviceList(devices) {
    const deviceList = document.getElementById('alexa-devices-list');
    if (!deviceList) return;
    
    deviceList.innerHTML = '';
    
    if (!devices || devices.length === 0) {
        deviceList.innerHTML = '<tr><td colspan="4">No devices configured</td></tr>';
        return;
    }
    ...
    devices.forEach(device => {
        ...
        row.innerHTML = `
            <td><input type="checkbox" ${device.enabled ? 'checked' : ''} onchange="toggleAlexaDevice(${device.id}, this.checked)"></td>
            <td>Relay ${device.id + 1}</td>
            <td>${device.name}</td>
            <td>
                <div class="action-btns">
                    <button class="btn btn-secondary btn-sm" onclick="editAlexaDevice(${device.id})">
...
// Update Alexa status UI based on enabled state
function updateAlexaStatus(enabled) {
    const statusMessage = document.getElementById('alexa-status-message');
    ...
    if (enabled) {
        ...
        statusMessage.innerHTML = `
            ...
            Alexa integration is enabled. You can discover your devices in the Alexa app.
        `;
    } else {
        statusMessage.className = 'error';
        statusMessage.innerHTML = `
            <i class="fa fa-times-circle"></i>
            Alexa integration is disabled. Enable it to control your relays with voice commands.
        `;
    }
...
// Set up the edit device modal
function setupAlexaDeviceEditModal() {
    ...
    // Form submission
    const form = document.getElementById('edit-alexa-device-form');
    if (form) {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            const deviceRow = document.querySelector(`#alexa-devices-list tr:nth-child(${parseInt(deviceId) + 1})`);
            if (deviceRow) {
                deviceRow.querySelector('td:nth-child(3)').textContent = deviceName;
                deviceRow.querySelector('.alexa-device-enabled').checked = enabled;
            }
            ...
            if (statusMessage) {
                statusMessage.className = 'success';
                statusMessage.textContent = `Device name updated. Don't forget to save your changes and rediscover devices in your Alexa app!`;
                statusMessage.style.display = 'block';
            }
    ...
    // Live update the example
    const deviceNameInput = document.getElementById('edit-device-name');
    if (deviceNameInput) {
        deviceNameInput.addEventListener('input', function() {
            const example = document.querySelector('.device-name-example');
            if (example) {
...
function openAlexaDeviceEditModal(deviceId, deviceName, enabled) {
    const modal = document.getElementById('edit-alexa-device-modal');
    if (!modal) return;
    
    document.getElementById('edit-device-id').value = deviceId;
    document.getElementById('edit-device-name').value = deviceName;
    document.getElementById('edit-device-enabled').checked = enabled;
    
    // Update the example
    const example = document.querySelector('.device-name-example');
    if (example) {
        example.textContent = deviceName;
    }
...
// Save Alexa configuration to the server
function saveAlexaConfig() {
    const enabledCheckbox = document.getElementById('alexa-enabled');
    ...
    .then(data => {
        if (data.status === 'success') {
            showToast('Alexa configuration saved successfully', 'success');
            loadAlexaConfig(); // Reload to ensure UI is in sync
        } else {
            showToast(`Failed to save configuration: ${data.message}`, 'error');
        }
    })
    .catch(error => {
        console.error('Error saving Alexa configuration:', error);
        showToast('Network error. Could not save configuration', 'error');
    });
}

// Toggle a single Alexa device
function toggleAlexaDevice(deviceId, enabled) {
    fetch('/api/alexa/config', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    .then(data => {
        if (data.status === 'success') {
            showToast(`Device ${deviceId + 1} ${enabled ? 'enabled' : 'disabled'}`, 'success');
        } else {
            showToast(`Failed to update device: ${data.message}`, 'error');
            // Reload to ensure UI is in sync
            loadAlexaConfig();
        }
    })
    .catch(error => {
        console.error('Error updating Alexa device:', error);
        showToast('Network error. Could not update device', 'error');
        // Reload to ensure UI is in sync
        loadAlexaConfig();
    });
}

// Edit an Alexa device name
function editAlexaDevice(deviceId) {
    fetch('/api/alexa/config')
        .then(response => response.json())
        .then(data => {
            ...
            const deviceNameField = document.getElementById('edit-device-name');
            const deviceEnabledField = document.getElementById('edit-device-enabled');
            const deviceNameExample = document.querySelector('.device-name-example');
            
            if (modal && deviceIdField && deviceNameField && deviceEnabledField && deviceNameExample) {
                deviceIdField.value = deviceId;
                deviceNameField.value = device.name;
                deviceEnabledField.checked = device.enabled;
                deviceNameExample.textContent = device.name;
...
// Save Alexa device edit
function saveAlexaDeviceEdit() {
    const deviceId = document.getElementById('edit-device-id').value;
    const deviceName = document.getElementById('edit-device-name').value;
    const deviceEnabled = document.getElementById('edit-device-enabled').checked;
...
// Trigger Alexa device discovery
function triggerAlexaDiscovery() {
    fetch('/api/alexa/discovery', {
        method: 'POST'
    })
    .then(response => response.json())
    .then(data => {
        if (data.status === 'success') {
            showToast('Device discovery triggered. Say "Alexa, discover devices" to complete the process', 'success');
        } else {
            showToast(`Failed to trigger discovery: ${data.message}`, 'error');
        }
    })
    .catch(error => {
        console.error('Error triggering discovery:', error);
        showToast('Network error. Could not trigger discovery', 'error');
    });
}